...
import { StatsCard, ActivityChart, CatchesMap, RecentCatches } from '../components/dashboard';
  ...
  const stats = useMemo(() => [
    {
      ...
      icon: Fish,
      up: true,
      change: t('dashboard.stats.totalCatches.thisMonth', { count: thisMonthCatches.length }),
    },
    {
      title: t('dashboard.stats.biggestCatch.title'),
      value: `${Math.max(...catches.map(c => c.catch.weight || 0))}g`,
      icon: Trophy,
      up: true,
    ...
    },
    {
      ...
      icon: Fish,
      up: true,
      change: t('dashboard.stats.thisYear.thisMonth', { count: thisMonthCatches.length }),
    },
    {
      title: t('dashboard.stats.locations.title'),
      value: new Set(catches.map(c => `${c.location.coordinates.lat},${c.location.coordinates.lng}`)).size,
      icon: MapPin,
      up: true,
      change: t('dashboard.stats.locations.heavyCatches', { count: heavyCatches.length }),
    },
